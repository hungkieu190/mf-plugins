...
class LP_WC_Hooks {
	...
	protected function hooks() {
		...
		add_filter( 'wcpay_payment_request_supported_types', function ( $types ) {
			$types[] = LP_COURSE_CPT;
			return $types;
		});
	...
	public function button_add_to_cart( $section, $course, $user ) {
		...
		if ( ! $course->is_free() ) {
			...
			if ( is_wp_error( $can_purchase ) && in_array( $can_purchase->get_error_code(), array( 'course_out_of_stock', 'course_purchased' ) ) ) {
				return $section;
			}
		}
