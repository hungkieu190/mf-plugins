...
class LP_UPPC_Metabox
{
				...
				array(
					...
					'fields' => array(
						...
						'target_courses' => new LP_Meta_Box_Select_Field(
							esc_html__('Target products', 'lp-upsell-progress-coupon'),
							esc_html__('Select courses to apply coupon to their WooCommerce products.', 'lp-upsell-progress-coupon'),
							[],
							array(
								'multiple' => true,
								'tom_select' => true,
								...
								'custom_attributes' => [
									'data-placeholder' => esc_html__('Search courses with products...', 'lp-upsell-progress-coupon'),
								],
							)
						),
	...
	public function save_course_metabox(int $post_id, WP_Post $post): void
	{
		...
		// If no rules data submitted, disable and clean up
		if (empty($data) || !is_array($data)) {
			delete_post_meta($post_id, '_lp_uppc_rules');
			update_post_meta($post_id, '_lp_uppc_enable_course', 'no');
			return;
		}
		...
		foreach ($data as $key => $row) {
			...
			$prepared[] = array(
				...
				'applies_to' => 'specific', // Always specific now
		...
		// Automatically enable course when rules are saved
		update_post_meta($post_id, '_lp_uppc_enable_course', 'yes');
	}
	...
	private function get_course_options(int $current_course_id): array
	{
		...
		if ($query->have_posts()) {
			foreach ($query->posts as $course_id) {
				// Only include courses that have WooCommerce products
