...
class LP_Sticky_Notes_Ajax
{
	...
	public function get_all_notes()
	{
		...
		foreach ($notes as $note) {
			$lesson_id = absint($note->lesson_id);
			$course_id = absint($note->course_id);
			$group_key = $course_id . ':' . $lesson_id;

			if (!isset($grouped_notes[$group_key])) {
				$grouped_notes[$group_key] = array(
					'lesson_id' => $lesson_id,
					'lesson_title' => $note->lesson_title,
					'lesson_url' => lp_sticky_notes_get_lesson_url($lesson_id, $course_id),
					'course_title' => $note->course_title,
					'notes' => array(),
				);
			}

			$grouped_notes[$group_key]['notes'][] = $note;
		}
