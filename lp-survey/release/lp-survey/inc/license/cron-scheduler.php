...
class LP_Survey_Cron {
    
    const CRON_HOOK = 'lp_survey_daily_license_check';
    ...
    public static function run_license_check() {
        ...
        if (defined('WP_DEBUG') && WP_DEBUG) {
            error_log('LP Survey: Daily license check - ' . ($is_valid ? 'VALID' : 'INVALID'));
        }
    ...
    private static function send_admin_notification() {
        ...
        $message = sprintf(
            "Your license for LearnPress Survey has failed validation.\n\n" .
            "This could mean:\n" .
            ...
            admin_url('admin.php?page=mamflow-license?tab=survey')
        );
...
add_action('init', function() {
    ...
    add_action(LP_Survey_Cron::CRON_HOOK, [LP_Survey_Cron::class, 'run_license_check']);
});
