<?php
/**
 * WP Cron Scheduler for LP Telegram Notifier
 * 
...
class MF_LP_Telegram_Cron
{
     ...
     * @var string
     */
    const CRON_HOOK = 'mf_lp_telegram_daily_license_check';
    ...
    public static function run_license_check()
    {
        ...
        if (defined('WP_DEBUG') && WP_DEBUG) {
            error_log('LP Telegram Notifier: Daily license check - ' . ($is_valid ? 'VALID' : 'INVALID'));
        }
    ...
    private static function send_admin_notification()
    {
        ...
        $subject = sprintf(__('[%s] License Validation Failed', 'lp-telegram-notifier'), $site_name);

        $message = sprintf(
            /* translators: %s: License management URL */
            __(
                "Your license for LP Telegram Notifier has failed validation.\n\n" .
                "This could mean:\n" .
            ...
            admin_url('admin.php?page=mamflow-license&tab=telegram-notifier')
        );
...
add_action(
    ...
    function () {
        ...
        add_action(MF_LP_Telegram_Cron::CRON_HOOK, array(MF_LP_Telegram_Cron::class, 'run_license_check'));
    }
);
